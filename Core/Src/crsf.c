...
static uint8_t CRSF_CalcCRC(uint8_t *data, uint8_t len) {
    uint8_t crc = 0;
    for (uint8_t i = 0; i < len; i++) crc = crsf_crc8_tab[crc ^ data[i]];
    return crc;
}
...
void CRSF_UpdateFailsafe(uint32_t current_time_ms) {
    if (crsf.is_connected && (current_time_ms - crsf.last_packet_time > CRSF_FAILSAFE_TIME)) {
        crsf.is_connected = false;
        crsf.link_quality = 0; // Лінк впав - LQ дорівнює нулю
        crsf.raw_channels[CRSF_MAP_THROTTLE] = 172;
        crsf.raw_channels[CRSF_MAP_STEERING] = 992;
    }
}
...
static void CRSF_ParseByte(uint8_t b) {
    ...
    if (current_time - last_byte_time > 5) {
        parser_state = CRSF_STATE_SYNC;
        frame_idx = 0;
    }
        ...
        case CRSF_STATE_PAYLOAD:
            ...
            if (frame_idx >= expected_len + 2) {
                ...
                if (crc_calc == crc_received) {
                    ...
                    // 1. Пакет стіків (RC Channels)
                    if (type == CRSF_FRAMETYPE_RC && expected_len == 24) {
                        CRSF_UnpackChannels(&frame_buf[3]);
                        crsf.last_packet_time = current_time;
                        crsf.is_connected = true;
                    }
                    // 2. Пакет статистики (Link Statistics)
                    else if (type == CRSF_FRAMETYPE_LINK_STATISTICS) {
                        // Згідно з протоколом CRSF:
                        // byte 3: Uplink RSSI Ant 1 (від'ємне значення, шлеться як додатне)
                        // byte 4: Uplink RSSI Ant 2
                        // byte 5: Uplink Link Quality (0 - 100%)
